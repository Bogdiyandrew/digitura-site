...
// Interfață extinsă pentru a acoperi ambele formulare
  ...
  try {
    ...
    if (!nume || !email) {
      return NextResponse.json(
        { message: 'Numele și email-ul sunt obligatorii.' }, 
        { status: 400 }
      );
    }
    ...
    // --- VALIDARE SPECIFICĂ FORMULARULUI ---
    if (formType === 'contact') {
    ...
    if (formType === 'contact') {
      ...
      emailHtml = `
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
          <div style="background-color: #0f172a; padding: 20px; text-align: center;">
            <h2 style="color: #3b82f6; margin: 0;">Cerere Nouă Audit Gratuit</h2>
          </div>
