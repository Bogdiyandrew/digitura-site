...
const Contact: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (packageParam) {
      ...
      const matchedPackage = pricingPackages.find(
        (pkg) =>
  ...
  const handleScrollToDirectContact = (e: React.MouseEvent) => {
    e.preventDefault();
    const element = document.getElementById('direct-contact');
    if (element) {
  ...
  const renderInputField = (
    ...
    return (
      <div className="relative group">
        <Icon
          className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-teal-400 transition-colors duration-300"
          size={18}
        />
        <input
          type={type}
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 w-full z-10">
        <div className="gsap-header text-center mb-10 md:mb-16 opacity-0">
          ...
          <p className="text-base sm:text-lg md:text-xl text-slate-300 max-w-2xl mx-auto leading-relaxed">
            Spune-ne pe scurt ce ai nevoie sau{' '}
            <a
              href="#direct-contact"
              onClick={handleScrollToDirectContact}
              className="text-teal-400 underline decoration-teal-400/50 hover:text-teal-300 cursor-pointer lg:no-underline lg:text-slate-300 lg:cursor-auto lg:pointer-events-none transition-colors"
            >
            ...
            <div className="mt-2">
              <div className="flex items-start gap-3">
                ...
                <label htmlFor="gdpr_consent" className="text-sm text-slate-400 cursor-pointer select-none leading-tight">
                  ...
                  și cu prelucrarea datelor cu caracter personal în scopul procesării cererii mele.
                </label>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    <div className={`p-2 md:p-3 rounded-full bg-${item.color}-500/10`}>
                      <Icon
                        className={`text-${item.color}-400 group-hover:scale-110 transition-transform duration-300`}
                        size={24}
                      />
                    </div>
                    ...
                    <Send
                      size={16}
                      className="text-slate-500 group-hover:text-teal-400 transition-all -rotate-45 group-hover:rotate-0 flex-shrink-0"
                    />
                  </a>
                );
              })}
